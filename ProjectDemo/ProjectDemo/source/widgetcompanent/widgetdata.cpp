...
//        handleMouseDbClickReleaseEvent(static_cast<QMouseEvent*>(event));
        break;
...
void WidgetData::resizeWidget(const QPoint &gMousePos)
{
    ...
    if (newRect.isValid())
    {
        ...
        {
//            qDebug()<<"origRect"<<origRect;
//            qDebug()<<" pre newRect"<<newRect<<m_pWidget;
            m_pWidget->setGeometry(newRect);
//            m_pWidget->resize(newRect.size());
//            qDebug()<<" back newRect"<<newRect<<m_pWidget;
        }
    }
}
...
void WidgetData::handleMouseDbClickReleaseEvent(QMouseEvent *event)
{
    if (event->button() == Qt::LeftButton)
    {
        m_bLeftButtonPressed = false;
        ...
        {
            m_pRubberBand->hide();
            if(!m_bDbClickPressed)
            {
                m_pWidget->setGeometry(QRect(m_pWidget->pos().x(),m_pWidget->pos().y(),
                                             m_pWidget->maximumSize().width(),m_pWidget->maximumSize().height()));
            }else{
                m_pWidget->setGeometry(QRect(m_pWidget->pos().x(),m_pWidget->pos().y(),
                                             m_pWidget->baseSize().width(),m_pWidget->baseSize().height()));
            }
            m_bDbClickPressed = !m_bDbClickPressed;
        }
    }
}
